...
!      iret = nf90_put_att(self%ncid,NF90_GLOBAL,'references','add references'); call check_err(iret)
   ...
   subroutine netcdf_variable_settings_initialize(self, settings, parent)
      ...
      call settings%get(self%xtype, 'xtype', 'data type', options=(/type_option(-1, 'default'), type_option(NF90_FLOAT, '32-bit float'), type_option(NF90_DOUBLE, '64-bit double')/), default=self%xtype)
